...
COMMENT ON TABLE sys_menu IS '系统菜单表（菜单与按钮权限共用）';
COMMENT ON COLUMN sys_menu.id IS '菜单ID';
COMMENT ON COLUMN sys_menu.parent_id IS '父级菜单ID（0表示一级菜单）';
COMMENT ON COLUMN sys_menu.menu_type IS '类型：1-菜单 2-按钮权限';
COMMENT ON COLUMN sys_menu.name IS '路由名称（唯一标识，如 SystemUser）';
COMMENT ON COLUMN sys_menu.path IS '路由路径（一级以/开头，子级不以/开头）';
COMMENT ON COLUMN sys_menu.component IS '组件路径（相对于 src/views，如 system/user/index）';
COMMENT ON COLUMN sys_menu.redirect IS '重定向路径';
COMMENT ON COLUMN sys_menu.icon IS '菜单图标';
COMMENT ON COLUMN sys_menu.title IS '菜单标题';
COMMENT ON COLUMN sys_menu.link IS '外部链接URL';
COMMENT ON COLUMN sys_menu.is_iframe IS '是否为iframe内嵌';
COMMENT ON COLUMN sys_menu.is_hide IS '是否在菜单中隐藏';
COMMENT ON COLUMN sys_menu.is_hide_tab IS '是否在标签页中隐藏';
COMMENT ON COLUMN sys_menu.is_full_page IS '是否全屏页面';
COMMENT ON COLUMN sys_menu.is_first_level IS '是否为一级菜单（无子菜单的独立页面）';
COMMENT ON COLUMN sys_menu.keep_alive IS '是否缓存页面';
COMMENT ON COLUMN sys_menu.fixed_tab IS '是否固定标签页';
COMMENT ON COLUMN sys_menu.show_badge IS '是否显示徽章';
COMMENT ON COLUMN sys_menu.show_text_badge IS '文本徽章内容';
COMMENT ON COLUMN sys_menu.active_path IS '高亮的菜单路径';
COMMENT ON COLUMN sys_menu.auth_name IS '按钮权限名称（menu_type=2时使用）';
COMMENT ON COLUMN sys_menu.auth_mark IS '按钮权限标识（menu_type=2时使用，如 btn_add）';
COMMENT ON COLUMN sys_menu.sort IS '排序（数值越小越靠前）';
...
-- 一级菜单：仪表盘
INSERT INTO sys_menu (id, parent_id, menu_type, name, path, redirect, icon, title, sort)
VALUES (1, 0, 1, 'Dashboard', '/dashboard', '/dashboard/console', 'dashboard', '仪表盘', 1);

-- 二级菜单：控制台、分析页
INSERT INTO sys_menu (id, parent_id, menu_type, name, path, component, title, sort)
VALUES
