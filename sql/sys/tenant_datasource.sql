...
CREATE TABLE sys.tenant_datasource (
...
CREATE UNIQUE INDEX uk_sys_tenant_datasource_tenant_id ON sys.tenant_datasource (tenant_id);
CREATE UNIQUE INDEX uk_sys_tenant_datasource_datasource_name
    ON sys.tenant_datasource (datasource_name)
    WHERE datasource_name IS NOT NULL AND datasource_name <> '';
CREATE UNIQUE INDEX uk_sys_tenant_datasource_schema_name
    ON sys.tenant_datasource (schema_name)
    WHERE schema_name IS NOT NULL AND schema_name <> '';
CREATE INDEX idx_sys_tenant_datasource_status ON sys.tenant_datasource (status);
CREATE INDEX idx_sys_tenant_datasource_isolation_level ON sys.tenant_datasource (isolation_level);
CREATE INDEX idx_sys_tenant_datasource_last_sync_time ON sys.tenant_datasource (last_sync_time);
...
COMMENT ON COLUMN sys.tenant_datasource.remark IS '备注';
COMMENT ON COLUMN sys.tenant_datasource.create_by IS '创建人';
COMMENT ON COLUMN sys.tenant_datasource.create_time IS '创建时间';
COMMENT ON COLUMN sys.tenant_datasource.update_by IS '更新人';
COMMENT ON COLUMN sys.tenant_datasource.update_time IS '更新时间';
