...
COMMENT ON COLUMN sys_operation_log.user_name IS '操作人用户名';
COMMENT ON COLUMN sys_operation_log.module IS '业务模块';
COMMENT ON COLUMN sys_operation_log.action IS '操作描述';
COMMENT ON COLUMN sys_operation_log.business_type IS '操作类型（0=其他, 1=新增, 2=修改, 3=删除, 4=查询, 5=导出, 6=导入, 7=授权）';
COMMENT ON COLUMN sys_operation_log.request_method IS 'HTTP请求方法';
COMMENT ON COLUMN sys_operation_log.request_url IS '请求URL';
COMMENT ON COLUMN sys_operation_log.request_params IS '请求参数（JSON，敏感接口不记录）';
COMMENT ON COLUMN sys_operation_log.response_body IS '响应内容（JSON）';
COMMENT ON COLUMN sys_operation_log.response_code IS 'HTTP响应状态码';
COMMENT ON COLUMN sys_operation_log.client_ip IS '客户端IP';
COMMENT ON COLUMN sys_operation_log.ip_location IS 'IP地理位置';
COMMENT ON COLUMN sys_operation_log.user_agent IS '浏览器User-Agent';
COMMENT ON COLUMN sys_operation_log.status IS '操作状态（1=成功, 2=失败, 3=异常）';
