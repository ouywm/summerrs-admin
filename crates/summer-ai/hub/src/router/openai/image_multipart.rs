...
use super::{
    BufferedMultipartField, apply_upstream_failure_scope, buffer_multipart_fields,
    build_image_edit_form, build_image_variation_form, build_json_bytes_response,
    classify_upstream_provider_failure, extract_request_id, extract_upstream_request_id,
    fallback_usage, insert_upstream_request_id_header, map_adapter_build_error,
    parse_image_edit_meta, parse_image_variation_meta, record_terminal_failure,
    spawn_usage_accounting_task, unusable_success_response_message,
};

#[derive(Clone, Copy)]
struct ImageMultipartEndpointSpec {
    upstream_path: &'static str,
    endpoint: &'static str,
    request_format: &'static str,
    default_model: &'static str,
    parse_meta: fn(&[BufferedMultipartField]) -> anyhow::Result<ImageMultipartMeta>,
    build_form: fn(&[BufferedMultipartField], &str) -> anyhow::Result<Form>,
}

#[derive(Debug, Clone)]
struct ImageMultipartMeta {
    model: String,
...
pub async fn image_edits(
    ...
    relay_image_multipart_request(
        ...
        headers,
        &mut multipart,
        ImageMultipartEndpointSpec {
            upstream_path: "/v1/images/edits",
            endpoint: "images/edits",
            request_format: "openai/images_edits",
            default_model: "gpt-image-1",
            parse_meta: parse_image_edit_meta_for_route,
            build_form: build_image_edit_form,
        },
    )
    .await
}
...
pub async fn image_variations(
    ...
    relay_image_multipart_request(
        ...
        ImageMultipartEndpointSpec {
            ...
            default_model: "gpt-image-1",
            parse_meta: parse_image_variation_meta_for_route,
            build_form: build_image_variation_form,
        },
    )
    .await
}
...
async fn relay_image_multipart_request(
    ...
    spec: ImageMultipartEndpointSpec,
) -> OpenAiApiResult<Response> {
    let request_id = extract_request_id(&headers);
    let user_agent = UserAgentInfo::from_headers(&headers).raw;
    let fields = normalize_image_fields(
        buffer_multipart_fields(multipart).await?,
        spec.default_model,
    );
    let meta = (spec.parse_meta)(&fields).map_err(|error| {
        OpenAiErrorResponse::internal_with("failed to parse image multipart metadata", error)
    })?;
    let requested_model = meta.model.clone();

    token_info
        .ensure_endpoint_allowed("images")
    ...
    let model_config = billing
        .get_model_config_for_endpoint(&requested_model, "images")
        .await
        .map_err(|e| OpenAiErrorResponse::from_api_error(&e))?;
    ...
    for attempt in 0..max_retries {
        ...
        let request_builder = match build_image_multipart_request_for_channel(
            http_client.client(),
...
fn normalize_image_fields(
    mut fields: Vec<BufferedMultipartField>,
    default_model: &str,
) -> Vec<BufferedMultipartField> {
    let has_model = fields
        .iter()
        .any(|field| field.name == "model" && field.filename.is_none());
    if !has_model {
        fields.push(BufferedMultipartField {
            name: "model".into(),
            filename: None,
            content_type: None,
            bytes: Bytes::from(default_model.to_string()),
        });
    }
    fields
}

fn parse_image_edit_meta_for_route(
    fields: &[BufferedMultipartField],
) -> anyhow::Result<ImageMultipartMeta> {
    let meta = parse_image_edit_meta(fields)?;
    Ok(ImageMultipartMeta {
        model: meta.model,
        estimated_tokens: meta.estimated_tokens,
    })
}

fn parse_image_variation_meta_for_route(
    fields: &[BufferedMultipartField],
) -> anyhow::Result<ImageMultipartMeta> {
    let meta = parse_image_variation_meta(fields)?;
    Ok(ImageMultipartMeta {
        model: meta.model,
        estimated_tokens: meta.estimated_tokens,
    })
...
fn build_image_multipart_request_for_channel(
    ...
    match channel_type {
        ...
        _ => Err(anyhow!("{} endpoint is not supported", spec.endpoint)),
    }
}
