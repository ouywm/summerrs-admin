...
use super::{
    apply_upstream_failure_scope, build_json_bytes_response, classify_upstream_provider_failure,
    extract_request_id, extract_upstream_request_id, fallback_usage,
...
pub async fn moderations(
    ...
    if body.get("model").is_none() {
        body["model"] = serde_json::Value::String(requested_model.clone());
    }
    let req: ModerationRequest = serde_json::from_value(body).map_err(|error| {
        OpenAiErrorResponse::internal_with("failed to deserialize moderation request", error)
    })?;

    token_info
        .ensure_endpoint_allowed("moderations")
    ...
    let model_config = billing
        .get_model_config_for_endpoint(&requested_model, "moderations")
        .await
        .map_err(|e| OpenAiErrorResponse::from_api_error(&e))?;
    ...
    let estimated_tokens = estimate_moderation_input_tokens(&req.input);
    ...
    for attempt in 0..max_retries {
        ...
        let request_builder = match build_moderation_request_for_channel(
            http_client.client(),
            ...
            Err(error) => {
                ...
                channel_svc.record_relay_failure_async(
                    ...
                    0,
                    format!("failed to build upstream moderations request: {error}"),
                );
                ...
                if attempt == max_retries - 1 || exhausted_after_exclusion {
                    ...
                    return Err(map_adapter_build_error(
                        "failed to build upstream moderations request",
                        error,
                    ));
                }
                continue;
        ...
        match request_builder.send().await {
            Ok(resp) if resp.status().is_success() => {
                ...
                let body_bytes = match resp.bytes().await {
                    ...
                    Err(error) => {
                        ...
                        channel_svc.record_relay_failure_async(
                            ...
                            elapsed,
                            0,
                            format!("failed to read upstream moderations response: {error}"),
                        );
                        ...
                        if attempt == max_retries - 1 {
                            ...
                            return Err(OpenAiErrorResponse::internal_with(
                                "failed to read upstream moderations response",
                                error,
                            ));
                        }
                        continue;
                ...
                if let Some(message) =
                    unusable_success_response_message(status, &body_bytes, "moderations", false)
                {
                    let _ = billing
                        .refund_with_retry(&request_id, token_info.token_id, pre_consumed)
                        .await;
                ...
                let (usage, upstream_model) =
                    moderation_usage_and_model(&body_bytes, estimated_tokens, &actual_model);
            ...
            Err(error) => {
                ...
                if attempt == max_retries - 1 {
                    record_terminal_failure(
                        ...
                        &user_agent,
                        0,
                        format!("failed to call upstream moderations endpoint: {error}"),
                    );
                    let _ = rate_limiter.finalize_failure_with_retry(&request_id).await;
                    return Err(OpenAiErrorResponse::internal_with(
...
fn build_moderation_request_for_channel(
    ...
    match channel_type {
        value if value == ChannelType::OpenAi as i16 || value == ChannelType::Azure as i16 => {
            let mut payload =
                serde_json::to_value(req).context("failed to serialize moderation request")?;
            payload["model"] = serde_json::Value::String(actual_model.to_string());
            let builder = client.post(build_upstream_url(base_url, "/v1/moderations", None));
            Ok(apply_upstream_auth(builder, channel_type, api_key).json(&payload))
        }
        _ => Err(anyhow::anyhow!("moderations endpoint is not supported")),
    }
}

fn moderation_usage_and_model(
    body: &[u8],
...
fn estimate_moderation_input_tokens(input: &serde_json::Value) -> i32 {
    ...
    match input {
        ...
        serde_json::Value::Array(items) => items
            .iter()
            .map(estimate_moderation_input_tokens)
            .sum::<i32>()
            .max(1),
        serde_json::Value::Object(map) => map
            .get("text")
            .map(estimate_moderation_input_tokens)
            .unwrap_or_else(|| estimate_text(&serde_json::Value::Object(map.clone()).to_string())),
