...
pub async fn image_generations(
    ...
    let req: ImageGenerationRequest = serde_json::from_value(body.clone()).map_err(|error| {
        OpenAiErrorResponse::internal_with("failed to deserialize image generation request", error)
    })?;
    ...
    for attempt in 0..max_retries {
        ...
        let request_builder = match build_image_generation_request_for_channel(
            http_client.client(),
            ...
            Err(error) => {
                ...
                channel_svc.record_relay_failure_async(
                    ...
                    0,
                    format!("failed to build upstream image generations request: {error}"),
                );
                ...
                if attempt == max_retries - 1 || exhausted_after_exclusion {
                    ...
                    return Err(map_adapter_build_error(
                        "failed to build upstream image generations request",
                        error,
                    ));
                }
                continue;
        ...
        match request_builder.send().await {
            Ok(resp) if resp.status().is_success() => {
                ...
                let body_bytes = match resp.bytes().await {
                    ...
                    Err(error) => {
                        ...
                        channel_svc.record_relay_failure_async(
                            ...
                            elapsed,
                            0,
                            format!("failed to read upstream image generations response: {error}"),
                        );
                        ...
                        if attempt == max_retries - 1 {
                            record_terminal_failure(
                                ...
                                0,
                                format!(
                                    "failed to read upstream image generations response: {error}"
                                ),
                            );
                            let _ = rate_limiter.finalize_failure_with_retry(&request_id).await;
                            return Err(OpenAiErrorResponse::internal_with(
                                "failed to read upstream image generations response",
                                error,
                            ));
                        }
                        continue;
                ...
                let (usage, upstream_model) =
                    image_usage_and_model(&body_bytes, estimated_tokens, &actual_model);
            ...
            Err(error) => {
                ...
                if attempt == max_retries - 1 {
                    record_terminal_failure(
                        ...
                        &user_agent,
                        0,
                        format!("failed to call upstream image generations endpoint: {error}"),
                    );
                    let _ = rate_limiter.finalize_failure_with_retry(&request_id).await;
                    return Err(OpenAiErrorResponse::internal_with(
    ...
    match channel_type {
        value if value == ChannelType::OpenAi as i16 || value == ChannelType::Azure as i16 => {
            let mut payload = serde_json::to_value(req)
                .context("failed to serialize image generation request")?;
            payload["model"] = serde_json::Value::String(actual_model.to_string());
            let builder = client.post(build_upstream_url(base_url, "/v1/images/generations", None));
            Ok(apply_upstream_auth(builder, channel_type, api_key).json(&payload))
        }
        _ => Err(anyhow::anyhow!(
            "image generations endpoint is not supported"
        )),
    }
}

fn image_usage_and_model(
    body: &[u8],
...
fn estimate_json_tokens(body: &serde_json::Value) -> i32 {
