...
struct FailureCooldownWindow {
    rate_limited_until: Option<chrono::DateTime<chrono::FixedOffset>>,
    overload_until: Option<chrono::DateTime<chrono::FixedOffset>>,
}

#[derive(Debug, Clone, PartialEq, Eq)]
struct SuccessHealthUpdate {
    next_channel_status: ChannelStatus,
    next_rate_limited_until: Option<chrono::DateTime<chrono::FixedOffset>>,
    next_overload_until: Option<chrono::DateTime<chrono::FixedOffset>>,
    invalidate_route_cache: bool,
}

#[derive(Debug, Clone, PartialEq, Eq)]
struct FailureHealthUpdate {
    penalize: bool,
    quarantine_account: bool,
    next_channel_failure_streak: i32,
    next_account_failure_streak: i32,
    next_channel_status: ChannelStatus,
    next_account_status: AccountStatus,
    next_account_schedulable: bool,
    next_health_status: i16,
    cooldown: FailureCooldownWindow,
    invalidate_route_cache: bool,
}
...
fn compute_test_success_health_update(
...
fn compute_relay_success_health_update(
    current_channel_status: ChannelStatus,
    rate_limited_until: Option<chrono::DateTime<chrono::FixedOffset>>,
    overload_until: Option<chrono::DateTime<chrono::FixedOffset>>,
    now: chrono::DateTime<chrono::FixedOffset>,
...
fn compute_failure_health_update(
    status_code: i32,
    current_channel_status: ChannelStatus,
...
fn should_penalize_upstream_health_failure(status_code: i32) -> bool {
    matches!(status_code, 0 | 408 | 409 | 425 | 429 | 500..=599)
}

fn should_quarantine_account_on_auth_failure(status_code: i32) -> bool {
    matches!(status_code, 401 | 403)
}

fn relay_request_started_at(
...
fn relay_health_update_is_stale(
...
mod tests {
    ...
    fn resolve_probe_endpoint_scope_rejects_resource_only_scopes() {
        let error =
            resolve_probe_endpoint_scope(1, &serde_json::json!(["files"]), None).unwrap_err();
    ...
    fn resolve_probe_endpoint_scope_rejects_requested_disabled_scope() {
        let error =
            resolve_probe_endpoint_scope(1, &serde_json::json!(["chat"]), Some("responses"))
                .unwrap_err();
        assert!(matches!(error, ApiErrors::BadRequest(_)));
    }
    ...
    fn compute_relay_success_health_update_does_not_reenable_auto_disabled_channel() {
        let now = chrono::Utc::now().fixed_offset();
        let update =
            compute_relay_success_health_update(ChannelStatus::AutoDisabled, None, None, now);
