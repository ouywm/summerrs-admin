...
mod tests {
    ...
    use super::{ChannelAccountRes, ChannelAccountStatus};

    #[test]
    fn channel_account_res_does_not_serialize_credentials() {
        let now = Utc::now().fixed_offset();
        let response = ChannelAccountRes {
            id: 1,
            channel_id: 10,
            name: "primary".into(),
        ...
        let json = serde_json::to_value(&response).expect("serialize channel account response");
