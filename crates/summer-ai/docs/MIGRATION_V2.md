...
**OpenAIResponses 入口**（`/v1/responses`）已完成首轮闭环：`OpenAIResponsesIngress` + `OpenAIRespAdapter` + `ServiceType::Responses/ResponsesStream` 已接通，请求会真实出站到 `/v1/responses`，并保留 `reasoning`、`previous_response_id`、built-in tools 与 endpoint scope 校验。
