...
│  - 框架层表名过滤(tables/skip_tables)       │
│  - RewriteConnection（非分片查询入口）      │
│  - ProbePlugin（调试/集成测试用探针）       │
│  - SQL 解析 / AST 操作辅助 helpers          │
└─────────────────────────────────────────────┘
                  ▲ 依赖
┌─────────────────────────────────────────────┐
│  sea-orm / sqlx                             │
...
- `summer-sql-rewrite` 不感知"分片 / 租户"等业务概念，只提供 pipeline + 框架层表名过滤
- 业务插件（乐观锁、自动填充、数据权限等）由业务方按需在应用层实现，不由库内置
...
2. DefaultSqlRewriter::rewrite()      ← sharding 内部改写（每个目标分片）：
   ...
   e. registry.rewrite_all()          ← 插件管道（summer-sql-rewrite）：
      框架层先做 tables()/skip_tables() 过滤，再调 plugin.matches()
...
**框架层统一处理**表名过滤（`tables()` / `skip_tables()`），插件 `matches()` 里只需关心
操作类型 / extensions 等业务条件——避免每个插件重复实现 `applies_to_table()`。
...
- 业务自定义类型（`CurrentUser` / `DataScope` / `OptimisticLockValue` 等）：
  web 中间件写入，业务插件读取
...
| 30-49 | 分片标记 | `TableShardingPlugin`（写分片路由注释） |
| 50-99 | 安全增强 | 用户实现的 乐观锁 / 自动填充 |
| 100+ | 业务定制 | 用户实现的 数据权限 / 其他业务插件 |

### 5.4 插件注册

通过 `summer-sql-rewrite` 提供的 `SqlRewriteConfigurator` 在 app 启动时注册：
...
App::new()
    ...
    .sql_rewrite_configure(|registry| {
        registry
            .register(MyOptimisticLockPlugin::new())
            .register(MyAutoFillPlugin::new())
            .register(MyDataScopePlugin::new())
    })
    .run()
    .await;
```

注册后所有 `ShardingConnection.execute_raw` / `RewriteConnection.execute_raw`
都会按 order 顺序应用全部 matched 插件。

`summer-sql-rewrite` 自带的可注册插件目前只有：

- `ProbePlugin`：探针插件，不改 SQL，只统计命中次数 + 写 `/* probe=N */` 注释。
  用于集成测试验证插件管道是否通了，也可作为新插件的最小模板参考。

---

## 六、业务侧推荐实现的插件（不在库内提供）

> 这些插件与具体业务强相关（列名约定、用户上下文模型、部门树结构…），
> 库不提供默认实现，由业务方按需在应用层实现。下面是常见模式参考。

### 6.1 租户行级注入

为 SharedRow 隔离的表自动注入 `WHERE tenant_id = ?`。
当前由 sharding 内部 `apply_tenant_rewrite` 完成，**不**走插件管道。
...
通过 `tables()` 限定目标表；从 `ctx.extension::<MyOptimisticLockValue>()` 取旧版本号。

### 6.3 审计字段自动填充 (`order≈60`)
...
数据从 `ctx.extension::<MyCurrentUser>()` 读取，需要 web 中间件先注入。

### 6.4 数据权限插件 (`order≈70`)

数据范围控制，从 `ctx.extension::<MyDataScope>()` 读取当前用户的可见范围：
...
│   ├── builtin/            内置插件
│   │   └── table_sharding.rs TableShardingPlugin（写分片路由注释）
│   └── helpers.rs          工具函数
│
├── connector/              连接抽象
...
2. `app/main.rs` 启动期通过 `summer_sql_rewrite::SqlRewriteConfigurator` 的
   `.sql_rewrite_configure(|registry| registry.register(...))` 注册
