...
use crate::{config::McpConfig, prompts};
...
impl AdminMcpServer {
    pub fn new(config: &McpConfig, db: DatabaseConnection) -> Self {
        Self {
            ...
            db,
            default_database_url: config.default_database_url.clone(),
        }
    }
    ...
    pub(crate) fn default_database_url(&self) -> Option<&str> {
        self.default_database_url.as_deref()
    }
...
impl ServerHandler for AdminMcpServer {
    ...
    async fn read_resource(
        ...
        match uri.as_str() {
            "schema://tables" => {
                let tables = table_tools::list_tables(self.db()).await?;
                ...
                let schema = table_tools::describe_table(self.db(), table)
                    ...
                    .map_err(|error| {
                        McpError::resource_not_found(
                            format!("resource `{uri}` not found: {error}"),
                            None,
                        )
                    })?;
