...
impl From<CreateShowcaseProfileDto> for biz_showcase_profile::ActiveModel {
    fn from(dto: CreateShowcaseProfileDto) -> Self {
        Self {
            ...
            cover_image: dto.cover_image.map(|value| Set(Some(value))).unwrap_or_default(),

            contact_name: dto.contact_name.map(|value| Set(Some(value))).unwrap_or_default(),

            contact_gender: dto.contact_gender.map(Set).unwrap_or_default(),

            contact_phone: dto.contact_phone.map(|value| Set(Some(value))).unwrap_or_default(),

            contact_email: dto.contact_email.map(|value| Set(Some(value))).unwrap_or_default(),

            official_url: dto.official_url.map(|value| Set(Some(value))).unwrap_or_default(),
            ...
            publish_date: dto.publish_date.map(|value| Set(Some(value))).unwrap_or_default(),

            launch_at: dto.launch_at.map(|value| Set(Some(value))).unwrap_or_default(),

            service_time: dto.service_time.map(|value| Set(Some(value))).unwrap_or_default(),

            attachment_url: dto.attachment_url.map(|value| Set(Some(value))).unwrap_or_default(),

            description: dto.description.map(|value| Set(Some(value))).unwrap_or_default(),

            extra_notes: dto.extra_notes.map(|value| Set(Some(value))).unwrap_or_default(),

            metadata: dto.metadata.map(|value| Set(Some(value))).unwrap_or_default(),
